...
class Section:
    ...
    def content(self):
        ...
        for line in lines:
            ...
            try:
                int(line[0])
                datetime.datetime.strptime(line.strip()[:10], "%d/%m/%Y")
                good = False
        ...
        if self.next_title is None:
            ...
            if self.title in content:
                ...
                data = content[
                    content.index(self.title.rstrip(string.digits))
                    + len(self.title.rstrip(string.digits))
                    + 1 :
                ]
            elif len(self.title) > 50 and self.title[:40] in content:
                data = content[content.index(self.title[:40]) + len(self.title[:40]) :]
            ...
            if self.title not in content or self.next_title not in content:
                ...
                data = content[start_index + len(self.title) + 1 : end_index]
            else:
                data = content[
                    content.index(self.title)
                    + len(self.title)
                    + 1 : content.index(self.next_title)
                ]

        # Remove tables of names from voting
        while (
            "The Dáil divided: Tá," in data
            or "The Committee divided: Tá," in data
            or "The Seanad divided: Tá," in data
        ):
            earliest = first_occuring(
                [
                    "The Dáil divided: Tá,",
                    "The Committee divided: Tá,",
                    "The Seanad divided: Tá,",
                ],
                data,
            )
    ...
    def __del__(self):
        attrs_to_del = ["non_header_content"]
    ...
    def non_header_content(self):
        return self.data[self.data.rindex("����������������������������������") :]
...
class PDF:
    ...
    def load(self):
        ...
        self.data = self.data.replace(
            "-----", " — "
        )  # interruption / continuation of interruption
    ...
    def section_headers(self):
        ...
        for header_line in header_lines:
            if "\x08" in header_line:
                if append_next:
                    cleaned_header_lines[-1] += (
                        " " + header_line[: header_line.index("�")]
                    )
                else:
    ...
    def debate_sections(self):
        ...
        for title, next_title in window(self.section_headers + [None], window_size=2):
            sections.append(Section(data=self.data, title=title, next_title=next_title))
        return sections
